...
function Documents({ company, documents }) {
    ...
    const searchDocuments = async (e) => {
        if (!e) return;
        const search = documents.filter((d) => d.title.toLowerCase().includes(keyword.toLowerCase()))
        console.log(search)
        setDocs(search)
    }
    ...
    return (
        ...
        <>
            <div className="flex gap-4">
                <Input type="text" className="w-full" onChange={(e) => setKeyword(e.target.value)} />
            ...
            <div className="flex">
                ...
                <div className="flex w-full px-4 justify-end">
                    <SlideOver
                        buttonTitle=<svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" fill="currentColor" class="bi bi-plus" viewBox="0 0 16 16"><path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z" /></svg>
                        title="Add Document"
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {docs.map((item, index) => (
                                            ...
                                            <tr key={index}>
                                                ...
                                                <td className="whitespace-nowrap py-4 px-3 text-sm text-gray-500">{item.size / 1000000} MB</td>
                                                <td className="whitespace-nowrap py-4 px-3 text-sm text-gray-500">{item.lastModified}</td>
