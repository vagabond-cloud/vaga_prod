...
const Domain = ({ isTeamOwner, workspace, host, hostname }) => {
  ...
  return (
    <AccountLayout>
      <Meta title={`Vagabond - ${workspace.name} | Domains`} />
      <Content.Title
        ...
        <Card>
          ...
          >
            <div className="flex items-center justify-between px-3 py-2 font-mono text-sm border rounded md:w-1/2">
              <div>
                <strong>{workspace[0].slug}</strong>
                <span className="pr-3">.{host}</span>
              </div>
              <Link href={`http://${workspace[0].slug + '.' + host}`} target="_blank">
                <ArrowTopRightOnSquareIcon className="w-5 h-5 cursor-pointer hover:text-red-600" />
              </Link>
            </div>
          </Card.Body>
        </Card>
        ...
        <>
          ...
          <Content.Container>
            ...
            {isLoading ? (
              ...
              <Content.Empty>
                Once you&apos;ve added your domain on Vagabond, that domain
                will show up here
              </Content.Empty>
            )}
          </Content.Container>
