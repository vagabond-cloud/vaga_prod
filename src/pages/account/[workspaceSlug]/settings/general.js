...
const General = ({ isTeamOwner, workspace }) => {
  ...
  const [name, setName] = useState(workspace[0]?.name || '');
  const [slug, setSlug] = useState(workspace[0].slug || '');
  ...
  const changeName = (event) => {
    ...
    api(`/api/workspace/${workspace[0].slug}/name`, {
      body: { name },
      method: 'PUT',
    }).then((response) => {
  ...
  const changeSlug = (event) => {
    event.preventDefault();
    setSubmittingState(true);
    api(`/api/workspace/${workspace[0].slug}/slug`, {
      body: { slug },
      method: 'PUT',
    }).then((response) => {
      setSubmittingState(false);
  ...
  useEffect(() => {
    ...
    setSlug(workspace[0].slug);
    setWorkspace(workspace);
  }, []);

  return (
    <AccountLayout>
      <Meta title={`Vagabond - ${workspace[0].name} | Settings`} />
      <Content.Title
        ...
        <Card>
          ...
          >
            <div className="flex items-center justify-between px-3 py-2 space-x-5 font-mono text-sm border rounded md:w-1/2">
              <span className="overflow-x-auto">{workspace[0].workspaceCode}</span>
              <CopyToClipboard
                onCopy={copyToClipboard}
                text={workspace[0].workspaceCode}
              >
