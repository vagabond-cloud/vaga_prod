...
const Team = ({ isTeamOwner, workspace }) => {
  ...
  return (
    <AccountLayout>
      <Meta title={`Vagabond - ${workspace.name} | Team Management`} />
      <Content.Title
                                ...
                                >
                                  <Menu.Items className="absolute right-0 z-20 mt-2 origin-top-right bg-white border divide-y divide-gray-100 rounded w-60">
                                    <div className="p-2">
                                      <Menu.Item>
                                        <button
                                          className="flex items-center w-full px-2 py-2 space-x-2 text-sm text-gray-800 rounded hover:bg-red-600 hover:text-white"
...
export const getServerSideProps = async (context) => {
  ...
  if (session) {
    ...
    if (workspace) {
      ...
      workspace[0].inviteLink = `${process.env.APP_URL
        }/teams/invite?code=${encodeURI(workspace[0].inviteCode)}`;
    }
  }
  ...
  return {
    props: {
      isTeamOwner,
      workspace: JSON.stringify(workspace[0]),
    },
  };
};
