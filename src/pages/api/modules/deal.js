import { createDeal, updateCompany } from '@/prisma/services/modules';
...
const handler = async (req, res) => {
    ...
    if (method === 'PUT') {
        ...
        const modules = await createDeal(session.user.userId, session.user.email, formInput, workspaceId, moduleId);
        if (modules.count === 1) {
